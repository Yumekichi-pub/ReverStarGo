...
function updateRankDisplay() {
  ...
  if (el) {
    ...
    el.innerHTML = `${rankIcon(idx, 56)} Rank ${idx + 1}: ${RANKS[idx].name}`;
  }
}
...
function showRankList() {
  ...
  html += `<div class="rank-list-title">📊 Rank List${_xmOn ? '<br><span style="font-size:0.7rem;color:#ff8060;">* Tap to change rank</span>' : ''}</div>`;
  html += '<div class="rank-list-scroll">';
  html += '<table class="rank-list-table">';
  html += '<thead><tr><th></th><th>Rank</th><th>Condition</th><th>Unlocks</th></tr></thead>';
  html += '<tbody>';
  for (let i = 0; i < RANKS.length; i++) {
    ...
    const condLine2 = (exam && exam.careerWins > 0) ? `<br><span style="font-size:0.7rem;color:#e0a050;">or ${careerPrefix}${exam.careerWins} cumulative wins</span>` : '';
    html += `<td>${RANKS[i].condition}${condLine2}</td>`;
    html += `<td>${unlockLabel}</td>`;
    html += '</tr>';
  }
  html += '</tbody></table>';
  html += '<p style="text-align:center;margin-top:8px;padding:4px 12px;">* RM = Reverse Match</p>';
  html += '</div>';
  html += '<button class="rank-list-close-btn" onclick="closeRankList()">Close</button>';
  html += '</div></div>';
