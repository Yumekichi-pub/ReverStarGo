...
// v148: アプリとして起動しているか（TWA / インストール済み PWA）を判定する。
...
// v148: Google Play への導線。すでにアプリで遊んでいる人には評価を、
// ブラウザで見ている人にはアプリ版の入手を案内する。
function updateStoreLink() {
  const el = document.getElementById('play-store-label');
  if (!el) return;
  el.textContent = isStandaloneApp() ? '★ Rate this app' : '▶ Google Play';
}
...
function showPage(page) {
  ...
  if (page === 'main') {
    ...
    if (isDailySetup && dailyChallengeDate) {
      ...
      titleEl.textContent = '📅 Daily - ' + m + '/' + d;
      if (tutBtn) tutBtn.style.display = 'none';
      battleSection.style.display = 'none';
      promoSection.style.display = 'none';
      backBtn.textContent = 'Return to Daily';
      ...
      backBtn.textContent = 'Return to Main';
