...
const MOVE_QUALITY_MESSAGES = {
  1: ['Brilliant!', 'Magnificent!', 'Excellent!', 'Masterful!', 'Beautiful move!'],
  2: ['Nice', 'Nice one', 'Good', 'Well done', 'Skillful'],
  3: ['OK', 'Good move', 'Not bad', 'Solid', 'Steady']
};

// v66: キャプチャ（囲み取り）称賛メッセージ
// {N} は実際の取り石数に置換される
const MQ_CAPTURE_MESSAGES = {
  large:  ['Amazing! {N} captured!', 'Clean sweep!', 'Massive haul!', 'Brilliant surround!', 'Total siege!'],   // 7個以上
  medium: ['Awesome!', 'Nice work!', 'Stunning!', 'Surround complete!', 'Well played!'],                       // 5-6個
  small:  ['Captured!', 'Yes!', 'Great move!', 'Surrounded!', 'Nice capture!']                  // 3-4個
};
...
function evaluateMoveQuality(q, r, s, player) {
  ...
  // v68: モード別の判定
  if (battleMode === 'cpu') {
    if (player !== humanColor) return null;       // CPU対戦時はプレイヤー手のみ
    if (cpuLevel < MQ_MIN_LEVEL) return null;     // Lv.3 以上のみ（CPU対戦時）
  } else if (battleMode === 'two') {
    if (!moveQualityTwoPlayerEnabled) return null; // 2人対戦は設定 ON のときのみ、両プレイヤー対象
  } else {
    return null;
  }
...
function showCaptureBonus(count, player) {
  ...
  // v68: モード別の判定
  if (battleMode === 'cpu') {
    if (player !== humanColor) return;             // CPU対戦時はプレイヤー手のみ
  } else if (battleMode === 'two') {
    if (!moveQualityTwoPlayerEnabled) return;      // 2人対戦は設定 ON のときのみ、両プレイヤー対象
  } else {
    return;
  }
