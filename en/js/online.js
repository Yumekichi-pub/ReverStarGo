...
function _olTagName(s) {
  return _olNorm(s.length >= 7 ? s.slice(0, 6) + '2' : s + '2');
}
...
function _olWatchHostRoom(code) {
  ...
  const tick = (first) => {
    ...
    if (online.transport && online.transport.isOpen && online.transport.isOpen()) {
      _olStatus(`Give your opponent the room code 【 ${code} 】 (waiting for them to join… press again to cancel)`);
      return;
    }
...
function _olDiagText() {
  try {
    ...
    if (!d) return '';
    const r = d.err === 'timeout' ? 'no response'
            : d.err === 'network' ? 'network error'
            : d.err === 'server-error' ? 'server not responding'
            : (d.err || 'unknown');
    return `\n(diagnostic: ${r} / ${d.tries} retries)`;
  } catch (e) { return ''; }
}
...
function olJoinPrompt() {
  ...
  if (online) { olTeardown(false); _olStatus('Cancelled'); olUpdateLobbyButtons(); return; }
  const input = prompt('Enter the 4-digit room code your opponent gave you');
  if (!input) return;
  const code = input.trim().toLowerCase(); // 旧コード（英字）も受け付ける
  if (code.length < 3) { alert('That room code is too short'); return; }
  _olBegin(code, false);
  _olStatus(`Connecting to room 【 ${code} 】…`);
  ...
  switch (m.t) {
    case 'hello':
      ...
      if (online.isHost) {
        ...
        _olStatus(`${online.oppName} is confirming the match format…`);
      }
      break;
    case 'start':
    ...
    case 'fmt':
      ...
      if (online.isHost) {
        ...
        _olStartGame();
        if (_changed) {
          _olBubble(`🤝 ${online.oppName} chose ${m.mode === 'reverse' ? 'Reverse Match' : 'a single game'}`, 'cp');
        }
      }
      break;
    case 'mv':
    ...
    case 'gpwait':
      ...
      _olBubble('💠 Waiting for CP call…', 'cp', { id: 'gpwait', sticky: true });
      break;
    case 'gpsel':
      ...
      _olBubble(m.c === 'black' ? '💠 CP call: Black' : '💠 CP call: White', 'cp');
      break;
    case 'ping':
...
async function _olHandleSync(p) {
  ...
  _olSaveSession();
  _olStartHeartbeat();
  if (!behind) _olBubble('🔌 Reconnected', 'cp');
}
...
async function _olApplySync(p) {
  ...
  try {
    ...
    console.error('[online] failed to restore the game', e);
  } finally {
...
function olLeaveRoom() {
  if (!confirm(`Leave room 【 ${online.code} 】?\nThe game will end and your opponent will be notified.`)) return;
  _olClearSession();
  olTeardown(true);
  _olStatus('You left the room');
  olUpdateLobbyButtons();
}
...
function olUpdateLobbyButtons() {
  ...
  h.textContent = inRoom ? '▶ Back to room' : 'Create room';
  j.textContent = inRoom ? '🚪 Leave room' : 'Join room';
  if (inRoom) _olStatus(`In room 【 ${online.code} 】 — opponent: ${online.oppName}`);
}
...
function _olAskFormat(hostMode) {
  ...
  const _clockLabel = (typeof CLOCK_PRESETS !== 'undefined' && typeof clockPreset !== 'undefined'
    && CLOCK_PRESETS[clockPreset]) ? CLOCK_PRESETS[clockPreset].label : 'None';
  const _clockLine = `<br><strong>⏱ Game clock: ${_olEsc(_clockLabel)}</strong>`;
  text.innerHTML = (isRm
    ? `${_olEsc(online.oppName)}'s room is set to<br><strong>Reverse Match (2 games)</strong>.`
    : `${_olEsc(online.oppName)}'s room is set to<br><strong>a single game</strong>.`)
    + _clockLine + `<br>Play with these settings?`;
  okBtn.textContent = isRm ? '⚔ OK, Reverse Match' : '⚔ OK, single game';
  altBtn.textContent = isRm ? 'Prefer a single game' : 'Prefer Reverse Match';
...
function _olFinishFormat(mode, changed) {
  ...
  _olStartGame();
  if (changed) {
    _olBubble(`🤝 Playing ${mode === 'reverse' ? 'a Reverse Match' : 'a single game'}`, 'cp');
  }
}
...
function _olStartGame() {
  ...
  _olChatBar(true);
  olUpdateLobbyButtons();
  showTurn(`🌐 Game on! You are ${online.myColor === 'black' ? '⚫ Black (first)' : '⚪ White (second)'}`);
}
...
async function _olApplyRemoteMove(q, r, s, gp) {
  ...
  try {
    ...
    console.error('[online] failed to apply the remote move', e);
  } finally {
...
function _olMaybeRematch() {
  ...
  const _msg = _rmContinuing
    ? `🌐 Reverse Match, game 2! You are ${online.myColor === 'black' ? '⚫ Black (first)' : '⚪ White (second)'}`
    : `🌐 Rematch! You are ${online.myColor === 'black' ? '⚫ Black (first)' : '⚪ White (second)'}`;
  showTurn(_msg);
}
...
function _olOnDisconnect(reason) {
  ...
  const oppName = online.oppName || 'Your opponent';
...
function _olStartReconnect(msg) {
  ...
  _olSaveSession();
  _olSetReconnectOverlay(msg || '🔌 Trying to reconnect…', true);
...
function _olGiveUpReconnect() {
  ...
  _olShowNotice(`⚡ Could not reconnect to ${oppName}\n(You can try again from "🔄 Reconnect to last game" in Game Setup)`);
}
...
async function olResumeSession() {
  ...
  online = {
    ...
    myColor: 'black', myName: _olNorm(p.name) || _olMyName(), oppName: _olNorm(p.oppName) || 'Opponent',  // v133
  ...
  olReturnToGame();
  _olStartReconnect('🔌 Connecting to your opponent…');
}
