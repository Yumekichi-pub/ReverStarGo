...
function selectTpMode(mode) {
  ...
  if (desc) {
    desc.textContent = mode === 'reverse'
      ? 'Each player takes Black once; the total stones over 2 games decide the winner (no first-move advantage)'
      : 'Each game decides a winner';
  }
...
function toggleTpNameMenu(color) {
  ...
  if (players.length === 0) {
    menu.innerHTML = '<div class="tp-name-menu-empty">No names yet</div>';
  } else {
...
function deleteTpSeries(pairIdx) {
  ...
  if (!confirm(`Delete all records for "${nA} vs ${nB}" (${count} games)?\nThis cannot be undone.`)) return;
...
function showTpRecords() {
  ...
  if (data.matches.length === 0) {
    body.innerHTML = '<p class="tp-empty">No records yet.<br>Enter player names and play —<br>your head-to-head records will appear here.</p>';
  } else {
    ...
    body.innerHTML = sorted.map((list, si) => {
      ...
      const rows = list.slice().reverse().map((m, i) => {
        ...
        if (m.t === 'rm') {
          ...
          res = m.r === 'd' ? 'Draw' : `${_tpEsc(m.r === 'a' ? m.a : m.b)} wins`;
        } else {
          const aIsBlack = (m.b === nA);
          aCell = aIsBlack ? `●${m.bs}` : `○${m.ws}`;
          bCell = aIsBlack ? `○${m.ws}` : `●${m.bs}`;
          res = m.r === 'd' ? 'Draw' : `${_tpEsc(m.r === 'b' ? m.b : m.w)} wins`;
        }
        return `<tr><td>${no}</td><td>${dd}</td><td>${aCell}</td><td>${bCell}</td><td>${res}</td></tr>`;
      }).join('');
      const drawTxt = draw > 0 ? ` / Draws: ${draw}` : '';
      return `
        <div class="tp-series">
          <div class="tp-series-head">⚔ ${_tpEsc(nA)} vs ${_tpEsc(nB)}
            <button class="tp-del-btn" onclick="deleteTpSeries(${si})" title="Delete records for this pair">🗑 Delete</button>
          </div>
          <div class="tp-series-total">Total: ${_tpEsc(nA)} ${winA} – ${_tpEsc(nB)} ${winB}${drawTxt}</div>
          <table class="tp-table">
            <thead><tr><th>#</th><th>Date</th><th>${_tpEsc(nA)}</th><th>${_tpEsc(nB)}</th><th>Result</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </div>`;
    }).join('');
