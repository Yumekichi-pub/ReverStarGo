...
async function onCellClick(q, r, s) {
  ...
  // 公開版 v75 を Premium に取り込み（西口さんの高校生のお孫さんが発見）。
...
function initGame() {
  ...
  // Premium-v34: 修行モード「師匠と修行」専用 undo/redo スタックもリセット
  if (typeof jukuUndoStack !== 'undefined') jukuUndoStack = [];
  if (typeof jukuRedoStack !== 'undefined') jukuRedoStack = [];
  ...
  if (battleMode === 'two') {
    ...
    // Premium-v10: 修行コースは trainingLevel (6/7/9/10) を表示。
    // ※ Premium-v11 で内部レベル体系が更新（MAX=Lv.8, FINAL=Lv.11）
    //   FINAL の cpuLevel=11 と太陽系 Lv.7 の cpuLevel=11 が被るので、
    //   body class で修行コース判定する isTrainingMode() を使う。
    let badgeText;
    if (typeof isTrainingMode === 'function' && isTrainingMode()) {
      badgeText = `Lv.${trainingLevel}`;
    } else if (cpuLevel >= 6) {
      badgeText = LEVEL_NAMES[cpuLevel-1];
    } else {
      badgeText = `Lv.${cpuLevel}`;
    }
  ...
  // Premium-v11: 修行コース中は「修行部屋に戻る」、それ以外は従来通り
  const restartBtnText = (typeof isTrainingMode === 'function' && isTrainingMode())
    ? '修行部屋に戻る' : 'ゲーム設定に戻る';
  document.getElementById('restart-btn').textContent = restartBtnText;
  // Premium-v14: ゲーム状態に応じて有効/無効を更新（試合中は構造的に押せなくする）
...
// Premium-v14: 「ゲーム設定に戻る」/「修行部屋に戻る」の有効/無効状態を更新する。
// 試合中（戻ると負け確定 or 修行中断）は disabled、それ以外は enabled。
// 公開版 v77 の同名ヘルパーを Premium に移植（_xmOn → devMode に置換）。
function updateRestartBtnState() {
  ...
  // Premium-v16: 修行コース中は戦績記録対象外なので、人間が1手も打っていなければ enable
  // （RM 進行中でも、修行コースなら handlePendingReverseMatchOnStart で1敗計上されないため安全）
  if (typeof isTrainingMode === 'function' && isTrainingMode()) {
    // Premium-v34: 「師匠と修行」モードはいつでも戻れる（途中までで止めて実戦に移れるように）
    if (typeof trainingMode !== 'undefined' && trainingMode === 'juku') {
      btn.disabled = false;
      return;
    }
    const playerNotMoved = !moveHistory.some(m => m.player === humanColor);
    btn.disabled = !playerNotMoved;
    return;
  }
  // RM 進行中（通常 RM）→ 試合中なので disable
  if (reverseMatch) { btn.disabled = true; return; }
  // ゲーム未開始 → enable
  if (!gameStarted) { btn.disabled = false; return; }
  // 開発者モード・チュートリアル中 → enable（影響なし）
  if (devMode || isTutorial || tutorialMiniGame) { btn.disabled = false; return; }
  // CPU対戦で人間がまだ1手も打ってない（v62 ルール）→ enable
  if (battleMode === 'cpu') {
    const playerNotMoved = !moveHistory.some(m => m.player === humanColor);
...
document.getElementById('restart-btn').addEventListener('click', (ev) => {
  ...
  // Premium-v34: 「師匠と修行」モードは戦績記録対象外なので、負けではなく中断確認だけ
  if (typeof trainingMode !== 'undefined' && trainingMode === 'juku') {
    quitMsg.innerHTML = '🪐 修行部屋に戻ると<br><strong>続きをすることができません</strong>。<br>よろしいですか？';
...
function confirmQuit() {
  ...
  reverseMatch = null;
  // Premium-v34: 「師匠と修行」モードは戦績記録対象外（純粋な学びの場）
  const _isJukuQuit = typeof trainingMode !== 'undefined' && trainingMode === 'juku';
  // 負けとして記録（CPU対戦・チュートリアル以外、開発者モード以外、師匠と修行モード以外）
  if (battleMode === 'cpu' && !isTutorial && !tutorialMiniGame && !devMode && !_isJukuQuit) {
