...
function selectBattleMode(mode) {
  ...
  // Premium-v101: 2人対戦のときだけプレイヤー名入力セクションを表示
...
function startGame() {
  ...
  // Premium-v33: 修行モード中はスキップ（修行コースに到達する頃には RM 体験済みのため）
  const inTraining = typeof isTrainingMode === 'function' && isTrainingMode();
  if (reverseMatch && typeof hasSeenRmIntro === 'function' && !hasSeenRmIntro() && !inTraining) {
...
async function executeMove(q, r, s, gpColor) {
  ...
  let _jukuKind = null;
  // Premium-v15 (v20 改): 「師匠と修行」モードのみ塾評価。それ以外は通常の「いい手」評価
  ...
  try {
    ...
    // Premium-v15: 修行コース中は塾トースト表示
    if (_jukuKind) {
      try { showJukuToast(_jukuKind); } catch (e) {}
    }
    ...
    if (groups.length > 0) {
      ...
      // Premium-v40: 修行モードではキャプチャアニメ（750ms × 取り石数）で師匠トーストが
      //   消えてしまうので、キャプチャ完了後に師匠トーストを再表示する。
      //   通常の「お手柄！」キャプチャ称賛は出さず、師匠の言葉で褒める運用。
      if (_inJukuMode && _jukuKind) {
        try { showJukuToast(_jukuKind); } catch (e) {}
      } else {
    ...
    //   そのまま CPU が打ってしまうと「悪い手→負ける→学べない」となるので、
    //   一度立ち止まらせて「考え直す」か「自分の判断で進む」を選んでもらう。
    if (_jukuKind === 'bad' && _inJukuMode && typeof showJukuBadConfirmModal === 'function') {
      const _choice = await showJukuBadConfirmModal();
      if (_choice === 'undo') {
        // Premium-v35: undoMove は冒頭で isAnimating チェックを行うため、
        // 「戻って考え直す」を押した瞬間に undo を有効化するには先に解除する
        isAnimating = false;
...
function colorLabel(color) {
  ...
  if (battleMode === 'two') {
    // Premium-v101: 名前が入力されていれば「黒・たけし」の形で表示
...
function cpuFatalNoGame() {
  ...
  if (reverseMatch && reverseMatch.round === 2) {
    extra = '\n（リバースマッチ2局目のやり直しです。1局目の結果はそのまま残ります）';
  }
  alert('⚠ うまく対局を続けられませんでした。\nこの対局は「ノーゲーム（無効）」とし、もう一度この局を最初からやり直します。' + extra);
  initGame();
}
