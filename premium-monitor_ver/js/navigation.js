...
function showPage(page) {
  ...
  // Premium-v9: ゲーム盤面以外のページに遷移する時は修行コースの背景クラスを除去
  // Premium-v18: テーマ復元処理は撤廃（修行コースのダークは初回のみ自動設定して
  // カスタム設定として永続化する方式に変更したので、復元の必要なし）
  ...
  // Premium-v3: 修行コース
  const trainingEl = document.getElementById('setup-training');
  ...
  if (page === 'main') {
    ...
    // Premium-v3: 修行コース「大宇宙の修行部屋」
    if (trainingEl) trainingEl.style.display = 'flex';
    // Premium-v8: 各コースの購入状態に応じて section の表示を切替
    if (typeof applyTrainingUnlocks === 'function') applyTrainingUnlocks();
    // Premium-v20: 修行コースのモード選択状態を反映
    //   trainingMode が設定済（ゲームから戻ってきた等）なら詳細画面を、未設定なら入口を表示
    if (typeof trainingMode !== 'undefined' && trainingMode) {
      if (typeof enterTrainingMode === 'function') enterTrainingMode(trainingMode);
    } else {
      const sel = document.getElementById('training-mode-select');
      const detail = document.getElementById('training-detail-area');
