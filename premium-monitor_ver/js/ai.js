...
function cpuChooseMove(validMoves, player) {
  ...
  // ===== Premium-v4: 修行コース AI =====
  // 「深さ解析index.html」で完成済みの実装を移植 (2026-05-02 11代目)
  // d1- (weighted 5:3:1): 太陽系 Lv.6
  //   1手読み (negamax depth=1) で全候補を評価し、上位3手を 5:3:1 の重みで weighted 選択
  //   (最善手56%, 2位33%, 3位11%) — 達人より上、MAX 未満の調度よい揺らぎ
  if (cpuLevel === 21) {
    ...
    for (const {move:[q,r,s]} of ranked) {
      ...
      scores.push({ move: [q,r,s], val });
    }
    scores.sort((a,b) => b.val - a.val);
    const top = scores.slice(0, Math.min(3, scores.length));
    const weights = [5, 3, 1].slice(0, top.length);
  ...
  // depth=1〜5 専用モード (level 11〜15): 修行コース 太陽系 Lv.7 / 銀河系 Lv.9-10
  //   level 11: depth=1 → 太陽系 Lv.7
  //   level 12: depth=2 → MAX 相当 (無料版 Lv.6 と同等)
  //   level 13: depth=3 → 銀河系 Lv.9
  //   level 14: depth=4 → 銀河系 Lv.10
  //   level 15: depth=5 → FINAL 相当 (無料版 Lv.7 と同等)
  if (cpuLevel >= 11 && cpuLevel <= 15) {
    const searchDepth = cpuLevel - 10;
    const ranked = validMoves.map(([q,r,s]) => ({
      move:[q,r,s],
      pri: positionValue(q,r,s) * 4 + evaluateMove(q,r,s,player)
    })).sort((a,b) => b.pri - a.pri);
    ...
    for (const {move:[q,r,s]} of ranked) {
      const val = withMove(q, r, s, player, () => {
        // 反復深化: TT キャッシュヒット効果で高速化（既存 Lv.6/7 と同パターン）
        let v = 0;
