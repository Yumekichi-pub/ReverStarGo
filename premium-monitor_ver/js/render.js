...
function countOnBoard(player) {
  // Premium-v12: CP セル [0,0,0] は盤面石数の得点に含めない（最大 36 マス固定）。
  //   囲み取りで CP の石を取った場合は captured.* に反映されるが、
  //   盤面に残っている CP の石は「合計」表示にはカウントしない。
  //   公開版 v79 を Premium に取り込み。
...
function updateScore() {
  ...
  if (blackR1El && whiteR1El) {
    if (reverseMatch && reverseMatch.round === 2 && reverseMatch.round1Result) {
      ...
      // Premium-v13: プレイヤー基準で表示する（色基準だと色入れ替え後に「あなた」側に
      // CPU の1局目得点が表示されてしまうバグを修正）。
      // 現在の humanColor が示す側＝「あなた」、反対側＝「CPU」として、
      // それぞれの 1局目得点を表示する。公開版 v72 を Premium に取り込み。
      ...
      // Premium-v107: 2人対戦リバースマッチの2局目もCPU版と同様に1局目の得点を表示
...
function render(validMoves, koRestrictedSet, koBlockedMoves) {
  // Premium-v110: オンライン対戦では相手の手番中はヒント（黄色い点）を出さない
