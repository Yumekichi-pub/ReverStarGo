...
const CLOCK_PRESETS = {
  off:   { label: 'なし',        main: 0,    byo: 0  },
  b30:   { label: '秒読み30秒',   main: 0,    byo: 30 },
  m3:    { label: '3分＋30秒',    main: 180,  byo: 30 },
  m10:   { label: '10分＋30秒',   main: 600,  byo: 30 },
  m15:   { label: '15分＋60秒',   main: 900,  byo: 60 },
};
...
function _clockNotifyOver(color) {
  const name = (typeof tpNameFor === 'function' && tpNameFor(color)) || (color === 'black' ? '黒' : '白');
  const msg = `⏰ ${name} の持ち時間がなくなりました（対局は続けられます）`;
...
function _clockRender() {
  for (const color of ['black', 'white']) {
    ...
    if (st.over) el.textContent = '⏰ 時間切れ';
